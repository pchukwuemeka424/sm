...
export default async function addProduct(state, formData) {
  try {
    ...
    if (!imageFile) {
      ...
      // Close file descriptor and attempt to delete the original uploaded image
      try {
        await fs.access(originalImagePath); // Check if the file exists and is accessible
        await fs.unlink(originalImagePath); // Attempt to delete the file
      } catch (unlinkError) {
